import SearchNav from "@/components/elements/SearchNav";
...
const Header = () => {
  ...
  return (
    <>
      <header className="rounded-full flex fixed items-center justify-between navbar-shadow bg-white mt-5 w-[85%] p-1 z-20">
          ...
          {navbarMenuList.map((nav) => (
            <ul key={nav.id}>
              <li className="flex gap-1 items-center justify-center">
                ...
                {nav.icon && <span className="ml-2 mb-1">{nav.icon}</span>}
              </li>
            </ul>
          ))}
        </div>
